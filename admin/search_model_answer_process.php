...
$sql = "SELECT id, title FROM model_answers WHERE department = ? AND title LIKE ?";
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        echo '<div class="download-link"><a href="download_model_answer.php?id=' . $row["id"] . '">Download PDF</a></div>';
        echo '</div>';
    }
} else {
