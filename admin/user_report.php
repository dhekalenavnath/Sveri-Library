...
<form id="f1" action="" method="post">
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $sql = "SELECT * FROM login_logout_logs WHERE user_id='$username'";
        $result = $conn->query($sql);

        if ($result->num_rows > 0) {
            echo "<h2>Student Login Report for $username</h2>";
            echo '<style>
            h2 {
                color: white;
            ...
            while ($row = $result->fetch_assoc()) {
                echo "<tr><td>{$row['user_id']}</td><td>{$row['login_time']}</td><td>{$row['logout_time']}</td></tr>";
            }
